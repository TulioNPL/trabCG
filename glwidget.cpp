...
void GLWidget::readOFFFile(const QString &fileName) {
    ...
    // Emite sinal quando o modelo de malha é carregado com sucesso
...
{
    destroyShaders();
    QString vertexShaderFile[] = {    // Caminho para arquivos shader
    ...
    QString fragmentShaderFile[] = {  // Caminho para arquivos shader
    ...
    vertexShader = new QGLShader(QGLShader::Vertex); // Cria objeto QGLShader para vértice
    if (!vertexShader -> compileSourceFile ( vertexShaderFile[currentShader])) // Arquivos QGLs são compilados
        qWarning() << vertexShader -> log();

    fragmentShader = new QGLShader ( QGLShader :: Fragment ); // Cria objeto QGLShader de fragmento
    if (!fragmentShader -> compileSourceFile (fragmentShaderFile[currentShader]))  // Arquivos QGLs são compilados
...
void GLWidget::createVBOs()
{
    ...
    // Metodo allocate() envia os dados do vértice ao VBO como dados contíguos não tipados
    // Por fim, os arrays de atributos são excluídos pois foram copiados para VBO
    delete[] vertices;
    vertices = NULL;
    ...
    vboTangents -> allocate ( tangents , numVertices *
    sizeof ( QVector4D )) ;

    delete[] tangents;
    tangents = NULL;

    // Criados objeto buffer para os índices dos triângulos da malha
